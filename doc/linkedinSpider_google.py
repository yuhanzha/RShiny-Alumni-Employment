...
if __name__ == '__main__':
    s = login(laccount='**************', lpassword='*********')  # 输入账号
    company_name = input('Input the company you want to crawl:')
    maxpage = 50  # 抓取前50页百度搜索结果，百度搜索最多显示76页

    # 百度搜索
    url = 'https://www.google.com/search?newwindow=1&biw=1536&bih=727&ei=A-d_WsLmJ4mm_QbQhKzQBQ&q='+company_name+'+site%3Awww.linkedin.com%2Fin'
    print(url)
    results = []
    failure = 0
